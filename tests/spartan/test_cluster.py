...
def test_cluster(
    ...
    filepath_list = [
        tmp_dir_path / f'{i}.txt' for i in range(n_made_files)
    ]
    commands = [f'touch ' + str(fp) for fp in filepath_list]
    q_commands = Queue()
    for c in commands:
        q_commands.put(c)
    ...
    for filepath in filepath_list:
        if filepath.exists():
            ...
            stdout = subprocess.run(ssh_command, encoding='utf-8',
